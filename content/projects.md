## Featured Projects

**[Arabica Social](https://alpha.arabica.social)**
Coffee journaling social app, built on the AT Protocol. [(source)](https://github.com/arabica-social/arabica)

**[Shutter](https://github.com/ptdewey/shutter)**
Approval-based snapshot testing library for Go.

**[Pendulum-nvim](https://github.com/ptdewey/pendulum-nvim)**
Neovim plugin that monitors coding duration and compiles metrics such as git project name, file type, and additional parameters, providing insightful data for productivity analysis.

## Other Projects

**[YankBank-nvim](https://github.com/ptdewey/yankbank-nvim)**
Neovim plugin that provides quick access to clipboard history.

**[DarkEarth-nvim](https://github.com/ptdewey/darkearth-nvim)**
A dark and earthy color scheme for Neovim.

**[PlantUML Language Server](https://github.com/ptdewey/plantuml-lsp)**
An implementation of the language server protocol (LSP) for PlantUML, providing autocompletion, definitions, and diagnostics for PlantUML diagrams. Also includes Neovim and VSCode plugins.

**[Blueprinter](https://github.com/ptdewey/blueprinter)**
CLI tool for quickly generating files from templates.

**[Bluesky Comments (Svelte)](https://github.com/ptdewey/bluesky-comments-svelte)**
Svelte component library allowing Bluesky comments to be used as comments for any web-page.

## Other Contributions

<!-- TODO: add blento and teal.fm contribs here at some point -->

**[gleam-lang/gleam](https://github.com/gleam-lang/gleam)**
Improved int/float exhaustiveness checks for case statements.
Fixed invalid JavaScript code generation from very large floats.

**[aaaton/golem](https://github.com/aaaton/golem)**
Fixed dictionary encoding issue where a zero-width byte order mark would be included in some outputs and added associated test cases.

**[Myriad-Dreamin/tinymist](https://github.com/Myriad-Dreamin/tinymist)**
Added and fixed Neovim related documentation.

**[Cyboard-DigitalTailor/zmk-keyboards](https://github.com/Cyboard-DigitalTailor/zmk-keyboards)**
Add support for additional keyboard layout (single-arc, three-column dactyl).

**[XAMPPRocky/tokei](https://github.com/XAMPPRocky/tokei)**
CLI app counting lines of code in a project. Added support for PlantUML.

**[rockerBOO/awesome-neovim](https://github.com/rockerBOO/awesome-neovim)**
Added additional Neovim plugins to Awesome list.

**[shoenot/witchesbrew](https://github.com/shoenot/witchesbrew.nvim)**
Improved build tooling and added additional highlight groups.

**[fredrikaverpil/godoc.nvim](https://github.com/fredrikaverpil/godoc.nvim)**
Added _fzf-lua_ support. Tested and provided review for experimental tree-sitter support.

**[karthik/wesanderson](https://github.com/karthik/wesanderson)**
Added _Asteroid City_-themed color schemes.
